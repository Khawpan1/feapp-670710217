  ...
  return (
    <div className="mx-auto max-w-md rounded-2xl bg-white p-6 shadow-md">
      <button onClick={handleNextClick}
              className="rounded-lg bg-cyan-600 px-4 py-2 font-semibold text-white mb-4">
        เรื่องถัดไป
      </button>

      {/* ส่วนแสดงรูปภาพโปสเตอร์ */}
      ...
      <button onClick={handleMoreClick} className="mt-2 text-sm text-cyan-600 font-medium">
        {showMore ? 'ซ่อนเรื่องย่อ' : 'อ่านเรื่องย่อ'}
      </button>
      {showMore && <p className="mt-2 text-slate-700 text-left">{movie.detail}</p>}
    </div>
  );
}
